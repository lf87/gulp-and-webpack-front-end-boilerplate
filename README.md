...
* **Critical CSS** - Run this seperately using `gulp critical` This plugin will inline "above the fold" CSS in to the head of your document. The CSS path(s) can be modified inside the config object within the gulpfile. You can check the output by running the `browser-sync` task. For help, refer to the documentation - https://github.com/addyosmani/critical
